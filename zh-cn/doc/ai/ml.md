...
1. 深度学习
	...
	从另外一个角度看，深度学习的多层结构实际上可以和多尺度多层次分析方法 (如小波分解，高斯金字塔分解) 相类比。
	每一层都可以看成是它底下一层的高阶函数(也是一种泛函的思想，自变量为多个函数)。
	如果是离散的情形，就是一种张量分析方法。
	正如前面所述，它本质上就是试图从高维世界寻找容易处理、性质良好的低维流形结构。
	高阶的函数表示特征空间，低阶的函数映射到高维特征空间，是一种点的重新部署，
	以便获得线性处理或其他性质良好的处理。
	而每一层函数的作用就是在不同尺度、不同层次上调制出最终的模型函数。

### 模型详解

由前面的讨论可知，机器学习由模型建立和求解最优化问题两个主要的步骤组成。

一般而言，机器学习处理的是连续的模型函数。

1. 优化的本质

	优化问题是以模型函数为自变量，某种指标值为应变量的函数。模型参数越复杂越容易过拟合。
	实践中往往引入正则化方法来降低模型参数的复杂性，以达到更好的泛化能力。
	从几何角度看，实际就是在优化空间中，给出限制性的区域，优化在此区域上进行。
	可以参考[拉格朗日乘子][Lagrange]的几何解释，以及[正则化][reg1]、[各种解释][reg2]、[直观理解][reg3]。

1. 神经网络模型的本质

	神经网络是一种用来拟合任何非线性函数的连续函数模型。
	它是由加权函数和激活函数组成，并进行多层连接。
	显然，加权函数在几何上就是超平面，是线性的。
	如果没有激活函数，线性函数的叠加只能得到线性函数，最终结果也是超平面，
	无法表达复杂的非线性函数。激活函数的作用就是改变其线性特性。
	图3-5给出了激活函数的示意图。

	![](ml-files/jh.png "激活函数示意图")

	图3-5 激活函数示意图

	图中的右半部分，横坐标是自变量，示意性地表示自变量(一般是向量)输入；
	纵坐标是超平面的应变量，表示线性变换后的结果。
	图中的左半部分，纵坐标是激活函数的自变量，对应着超平面的输出；
	横坐标是应变量，对应着非线性变换的输出。观察左右两个横坐标，
	并分析自变量和应变量之间的关系，可以看到经过上述两步变换后，
	非线性变换使得变量对应点的距离发生了改变，有不变、增大和减小三种情况；
	对应点之间的大小顺序也有不变和逆序改变两种情况(相当于有负反馈)。
	最终的深度神经网络模型正是由大量这样的函数(定义域是右半部分横坐标，全数域)
	经过两步变换后做加法得到最终的模型。由于存在上述各种效应，
	所以能拟合逼近任意复杂的非线性函数。

	超平面和激活函数是简单的函数，基本趋势是单调增或减。正如[顾险峰][gu-ai]所说，
	深度神经网络是由足够简单的子群(超平面和激活函数)来逼近任意非线性函数。
	这在计算机上的处理是变得简单了。但对于要拟合的复杂的非线性函数，
	比如有大量鞍点、峰和谷，如图3-6所示：

	![](ml-files/nh.png "拟合过程示意图")

	图3-6 拟合过程示意图

	在拐点附近，使用少量单调增减的函数是无法逼近的，
	必然要组合使用更多的逆序变换函数来遏制先前单调增/减的趋势。
	而组合使用的简单函数越多，逼近的效果就更好。
	这也就是为什么深度学习模型要使用巨量参数的原因。同时这也带来了大量能源消耗的副作用。

1. 镜像变换

	从图3-5还可以看出，由于激活函数本身具有的对称性，所以只要列出半个圆的情形，
	就可以覆盖所有情况，刚好是八卦图的一半。如果激活函数改为基于y轴的不对称性形状，
	如图中蓝色线所示，那么仅用半个圆的情形是不能覆盖所有情况的，
	必须做一次镜像变换，如左下角的图中红色线所示，这样继续推演，得到圆的另外一半，
	就可以覆盖所有情况。此时形成了完整的八卦图。
	所以八卦图比目前普遍使用的深度学习激活函数多了个镜像变换的考虑。

	将激活函数改为不对称形状，在拟合拐点附近的曲线时，可以更快地遏制单调增/减的趋势，
	最终就能减少模型的总体参数个数。可以进一步推导，增加激活函数更多的不对称特性，
	或者给激活函数的设计加上更多的物理意义，就能用更少的参数达到类似的效果，
	从而使得模型更实用。中国古代从八卦到六十四卦的推演正是遵循上述思路的一种途径。
	可以进一步地从这个理论出发，探索更多的途径。
...
[Lagrange]: http://blog.sciencenet.cn/blog-3428464-1283235.html "拉格朗日乘子法"
[reg1]: https://zhuanlan.zhihu.com/p/29360425
[reg2]: https://mp.weixin.qq.com/s?__biz=MzU0MDQ1NjAzNg==&mid=2247541375&idx=1&sn=2c4e882d69e0cbec156eec71a38cf599&chksm=fb3a8774cc4d0e62899bb78208be93d1bd2fe3798090c00c9a9ae078a2cd3bd98a6444594cb4&scene=27
[reg3]: https://blog.csdn.net/jinping_shi/article/details/52433975
[gu-ai]: https://ishare.ifeng.com/c/s/v002JgwF4NYcbmQ82S4-_eS-_3--f5R3EL7B4JxV5Rtmq7LCNA__ "顾险峰AI科普"
